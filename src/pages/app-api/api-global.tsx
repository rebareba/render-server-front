...
class ApiGlobal extends React.Component {
  ...
  @action.bound deleteItem(deleteIndex, item) {
    const {store} = this.props
  ...
  render() {
    ...
    const paramsColumns = [
      ...
      },
      {
        ...
        render: (text, item, index) => {
          const props = {
            placeholder: '请填写参数名称',
            onChange: (e) => {
              this.handleOnChange(e.target.value, 'name', item, index)
            },
      ...
      },
      {
        ...
        render: (text, item, index) => {
          const props = {
            ...
            onChange: (value) => this.handleOnChange(value, 'paramLocation', item, index),
          }
      ...
      },
      {
        ...
        render: (value, item, index) => {
          const props = {
            ...
            onChange: (e) => this.handleOnChange(e.target.value, 'description', item, index),
      ...
      },
      {
        ...
        width: 50,
        render: (value, item, index) => {
          return <a onClick={() => this.deleteItem(index, item)}>删除</a>
        },
      },
    ]
    console.log(store.globalConfig)
    /** HTTP类型的入参和映射 */
